            ...
            <div class="col-6">
                <form method="POST" action="{{ route('trainings.update', $training->training_id) }}">
                    ...
                    <div class="form-group mb-2">
                        ...
                        <input id="date" name="date" type="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $training->date) }}" required min="2024-01-01">
                        @error('date')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group mb-2">
                        ...
                        <select id="trainer_id" name="trainer_id" class="form-select @error('trainer_id') is-invalid @enderror" required>
                            ...
                            @foreach ($trainers as $trainer)
                                <option value="{{ $trainer->user_id }}" {{ $trainer->user_id == $training->trainer_id ? 'selected' : '' }}>{{ $trainer->name }} {{ $trainer->surname }}</option>
                            @endforeach
                        </select>
                        @error('trainer_id')
                        <div class="invalid-feedback">{{ $message }}</div>
